// api/notify-live.js  — Vercel serverless function
...
module.exports = async function handler(req, res) {
  ...
  const emailHtml = `
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;padding:24px;background:#0d0d0d;color:#fff;border-radius:12px">
      ...
      <p style="color:#ccc;margin:0 0 24px">Jacob just went live. Join the trading mentorship stream now — don't miss it.</p>
      <a href="${streamUrl}"
  ...
  for (let i = 0; i < emails.length; i += CHUNK) {
    ...
    const payload = chunk.map(to => ({
      from,
      to: [to],
      subject: '🔴 Jacob is LIVE on The Greenprint right now!',
      html: emailHtml
    }));
