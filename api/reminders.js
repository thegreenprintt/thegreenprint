...
// Pinged every ~10 min by a GitHub Actions cron. Each call checks today's
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    let items = (SCHED[dayKey] || []).map(a => ({ t: a[0], c: a[1], title: a[2], host: a[3], feat: a[4] || false })).filter(qualifies);
    const seen = new Set();
